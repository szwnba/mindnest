...
test.describe("九型人格 /enneagram", () => {
  test("页面加载 200 + 显示 Quiz 组件", async ({ page }) => {
    const res = await page.goto("/enneagram");
    expect(res?.status()).toBe(200);

    // Quiz 区域存在
    await expect(page.locator(".quiz-enneagram")).toBeVisible();
    // 介绍文本
    await expect(page.getByText(/核心动机/)).toBeVisible();
  });
});
...
test.describe("使用条款 /terms", () => {
  test("页面加载 200 + 显示核心内容", async ({ page }) => {
    ...
    await expect(page.getByText(/使用本网站/)).toBeVisible();
  });
});
...
test.describe("人格档案 /profile", () => {
  test("页面加载 200 + 空状态提示", async ({ page }) => {
    ...
    // 无历史记录时显示空状态
    await expect(page.getByText(/开始你的第一次测评|还没有测评记录/)).toBeVisible();
  });
});

test.describe("分享结果 /share", () => {
  test("页面加载 200 + 显示分享卡片", async ({ page }) => {
    ...
    // 有分享按钮
    await expect(page.getByText(/分享给朋友|Share on X/)).toBeVisible();
    // 有返回首页链接
    await expect(page.getByText(/开始测评|再测一次/)).toBeVisible();
  });
