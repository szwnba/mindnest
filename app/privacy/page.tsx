...
export const metadata: Metadata = {
  ...
  description:
    `${SITE_NAME} 的隐私政策。我们承诺：所有数据仅存储在浏览器本地，无账号系统，无服务端存储，无第三方追踪。`,
  alternates: {
    canonical: "/privacy",
  },
};
    ...
    <>
      ...
      <main id="main" className="container narrow" style={{ paddingTop: "3rem", paddingBottom: "4rem" }}>
        <article className="prose">
          <h1>隐私政策</h1>
          <p className="lead">
            在${SITE_NAME}，我们相信隐私是基本人权。本政策详细说明我们如何（以及为何不）处理你的数据。
          </p>

          <h2>核心原则：你的数据只属于你</h2>
          <p>
            ${SITE_NAME} 的所有测评结果<strong>仅保存在你当前浏览器的本地存储中</strong>（sessionStorage 和 localStorage）。
          ...
          <h2>Cookie 使用</h2>
          <p>
            ${SITE_NAME} 不使用任何 Cookie。我们不需要 Cookie 来维持登录状态或追踪用户行为。
          </p>

          <h2>第三方服务</h2>
          <p>
            本网站托管在 Vercel 平台上。Vercel 可能会收集基本的访问日志（如 IP 地址、访问时间、User-Agent），
            但这些数据由 Vercel 根据其自身的隐私政策处理，与${SITE_NAME} 无关。
          </p>
